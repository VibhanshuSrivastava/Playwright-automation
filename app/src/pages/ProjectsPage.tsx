...
function ProjectsPage() {
  ...
  useEffect(() => {
    // Standard fetch-on-mount: intentional, one-time initial load.
  ...
  const handleUpdateProject = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const data = await updateProject(editingProjectId, editName, editDescription, editStatus);

      setProjects((currentProjects) =>
        currentProjects.map((project) => (project.id === editingProjectId ? data : project)),
      );
      ...
      {projects.length === 0 ? (
        ...
        <div className="projects-list">
          {projects.map((project) => (
            <div className="project-card" data-testid={`project-card-${project.id}`} key={project.id}>
                ...
                <form onSubmit={handleUpdateProject}>
                  ...
                  <div className="form-group">
                    ...
                    <select
                      ...
                      value={editStatus}
                      onChange={(event) => setEditStatus(event.target.value as 'active' | 'completed')}
                    >
                  ...
                  <button type="button" onClick={() => setEditingProjectId(null)} disabled={updating}>
                    Cancel
                  </button>
                </form>
              ) : (
